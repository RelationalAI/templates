...
- A continuous decision variable (`Order.quantity`) for how much to buy through each supply option.
...
Then it declares an `Order` decision concept and builds a continuous optimization model. The solver chooses `Order.quantity`, enforces capacity and demand with `require(...)`, and minimizes cost (optionally adding a reliability penalty):
