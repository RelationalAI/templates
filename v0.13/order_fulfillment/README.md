...
Finally, it solves with `Solver("highs")` and prints the assignments and the active fulfillment centers (with output filters `Assignment.qty > 0.001` and `FCUsage.used > 0.5`):
...
<details>
  ...
  - The output is filtered to `Assignment.qty > 0.001`; very small assignments will not display.
