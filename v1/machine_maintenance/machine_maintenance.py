...
linkage_constraint = model.require(
    assign_per_mp == MachinePeriod_outer.x_maintain
).where(MachinePeriod_outer.machine(Machine_ref), MachinePeriod_outer.period(Period_outer))

# C3: Technician hours capacity.
# Total assigned maintenance hours per technician per period must not
# exceed their available hours (availability * max_weekly_hours).
assigned_hours = sum(
...
avail_hours = sum(TechnicianPeriod_ref.capacity_hours).where(
...
# C4: Parts and bay capacity per period.
# At most PARTS_CAPACITY_PER_PERIOD maintenance jobs in any single period.
maint_per_period = sum(MachinePeriod_cap.x_maintain).where(
    MachinePeriod_cap.period(Period_cap)
).per(Period_cap)

capacity_constraint = model.require(maint_per_period <= PARTS_CAPACITY_PER_PERIOD)
...
total_cost = failure_cost + labor_cost + travel_cost
...
s.satisfy(coverage_constraint)
s.satisfy(linkage_constraint)
s.satisfy(hours_constraint)
s.satisfy(capacity_constraint)
...
for period, g in maint_df.groupby("period"):
    ...
    for _, row in g.iterrows():
        print(f"    {row['machine_id']} ({row['type']}, {row['facility']}, crit={int(row['criticality'])})")
...
for period, g in assign_df.groupby("period"):
    ...
    for _, row in g.iterrows():
        ...
        print(f"    {row['machine_id']}: {row['technician_id']} "
