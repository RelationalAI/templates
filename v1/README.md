...
| [money_laundering_motif_detection](./money_laundering_motif_detection/) | Detect smurfing fan-in patterns in a transaction graph using a CSP solver: K under-threshold transactions from a single beneficial-owner cluster into one destination, all within a tight time window. |
