...
| ~~[inventory_rebalancing](./inventory_rebalancing/)~~ | Moved — multi-tier flow conservation is now part of [`network_flow_planning`](./network_flow_planning/). |
...
| ~~[order_fulfillment](./order_fulfillment/)~~ | Moved — fixed-cost fulfillment-center selection is now part of [`network_flow_planning`](./network_flow_planning/). |
| [network_flow_planning](./network_flow_planning/) | Plan a multi-tier distribution flow that decides which fulfillment centers to open and how much to ship on every lane to satisfy customer demand at minimum cost. Combines flow conservation, fixed-cost facility opening, and capacity-gated open decisions in a single MILP. |
