...
| [money_laundering_motif_detection](./money_laundering_motif_detection/) | Detect layering "butterfly" patterns in a transaction graph using a CSP solver: a source routing under-threshold amounts through K beneficial-owner-clustered hubs to a single destination, with per-hub flow conservation in dollar amounts. |
