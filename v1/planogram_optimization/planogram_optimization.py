...
- Three integer decisions per SKU: `facings` in `[0, max_facings]`,
  ...
  coupled to `facings` by a pair of linear inequalities so the per-category
...
- Capacity, cardinality, and the active-iff-facings coupling are pure
  ...
  implies-bodied table lookup is solver-only; a post-solve pandas check
  re-validates `(sku_id, facings) -> demand_units` against the input table.

Run:
    `python planogram_optimization.py`

Output:
    Prints the formulation, the optimal facings per SKU, per-shelf utilization,
...
# cardinality ICs and the solver is free to leave them inactive.
def _assert_categories_cover_skus(skus_csv, categories_csv):
    ...
    if missing:
        raise ValueError(
            ...
            "Add a row per SKU category with `min_skus_active`/"
            "`max_skus_active` bounds so every SKU is covered by the "
...
_assert_categories_cover_skus(skus_csv, categories_csv)
...
# holds. The lookup is verified post-solve via a pandas anti-join below.
problem.verify(
    shelf_capacity_ic,
    category_min_ic,
    category_max_ic,
...
if mismatches:
    raise AssertionError(
        "Demand-lookup verification failed: Sku.realized_demand does not match "
        f"PredictedDemand.demand_units at the chosen facings count for "
        f"{len(mismatches)} SKU(s) "
        "(rows: (sku_id, facings, realized_demand, expected_demand)):\n"
        f"{mismatches}"
    )
