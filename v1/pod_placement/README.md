...
6. Expected output (the solver maximises the number of placed deployments; the specific (pod, node) tuples and per-node utilisation may vary across solver versions when multiple placements tie at the optimum). The script first prints the formulation (a few thousand lines for the bundled data, omitted here for brevity), then the solve-result block, then the per-node utilisation, the per-pod placement, and any unplaced deployments. A representative run looks like:
   ...
   The maximum feasible objective is `11` of `12` deployments: `ml_gamma_train` (4 pods, 4 GPUs, rack-clique IC) is left unplaced because of the interaction of three constraints -- gang placement (all 4 replicas or none), rack-clique (all 4 must share a rack), and CPU bin-packing on the GPU hosts (each GPU node has 8000m CPU; four ml pods need exactly 8000m, but the GPU hosts also have to absorb part of the rest of the cluster's load). Relaxing any one of the three (or dropping the `db_gamma` spread requirement) lets `ml_gamma_train` slot in and pushes the objective to `12`. The bound counts that remain solver-invariant across the optimum: each *placed* multi-replica deployment lands at exactly `ceil(replicas / 2)` pods per zone (`web_*` 4+4, `api_*` 2+2, `db_gamma` 3+3 -- the spread cap binds for every placed deployment with > 1 replica; `ml_gamma_train` is the one excluded by being unplaced), the affinity-paired caches (`cache_alpha`, `cache_delta`) co-locate on a single node, alpha/beta and gamma/delta tenant pods partition the cluster into disjoint node sets, and at least one GPU-adjacent node hits 100% CPU. Specific (pod, node) assignments may differ across runs when multiple placements tie at objective=11.
...
- Distributed-training rack-cliques require a single rack with enough capacity for the full group. If a 4-pod training job needs 4 GPUs and no single rack has 4 GPUs on hosts that can also satisfy the bin-packing IC, the rack-clique IC + the gang IC together rule out a fractional placement -- the deployment is left fully unplaced and the objective is `11`, not `12`, on the bundled data.
...
- The optimal solution can leave a deployment unplaced if placing it would require violating another IC. Check the printed "Unplaced deployments" block. The most likely binding constraints are (in order of frequency): rack-clique on a distributed-training group whose GPU requirement exceeds any single rack's capacity; gang placement (the deployment's replicas can't all fit somewhere); failure-domain spread (the deployment has more replicas than `num_zones * largest_single-zone-capacity`).
...
- Background: a MIP backend (HiGHS) is technically feasible -- the pairwise anti-affinity reads as a standard `x[p,n] + x[q,n] <= 1` exclusion (a set-packing constraint) over the existing binaries, and no big-M is required for that piece. The reason this template targets CP / MiniZinc anyway is that the gang reified cardinality and the topology-clique constraints are most naturally expressed as CP propagators; CP solvers typically prune them more aggressively than an LP relaxation for the kind of discrete structure this problem has.
