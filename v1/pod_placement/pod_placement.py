...
# duplicate keys (would collapse rows), dangling foreign keys (would
# silently drop pods/deployments from joins), non-positive capacities
# (would make bin-packing vacuous), and replica-count mismatches
# (gang-placement IC would force an unsolvable cardinality).
# --------------------------------------------------
...
def _assert_no_dangling_fks(child_df, child_col, parent_df, parent_col, source, parent_source):
    ...
    dangling = sorted({v for v in child_df[child_col].unique().tolist() if v not in parent_ids})
    if dangling:
        raise ValueError(
    ...
    for c in cols:
        ...
        if bad:
            raise ValueError(
                ...
                f"a zero or negative value silently breaks the IC that "
                f"consumes {c} (capacity columns turn the bin-packing IC "
                f"vacuous and hide overload; count columns make the "
                f"gang-placement IC trivially satisfied as 0 == 0 for any "
                f"placement, allowing spurious unplaced rows)."
            )
    ...
    for c in cols:
        ...
        if bad:
            raise ValueError(
                ...
                f"a negative value distorts the {c} bin-packing IC "
                f"(negative capacity makes the node unassignable for any "
                f"pod; negative demand reduces the effective sum and hides "
                f"real overload)."
            )


def _assert_replicas_match(dep_df, pod_df):
    expected = {int(r.deployment_id): int(r.replicas) for r in dep_df.itertuples(index=False)}
    actual = pod_df.groupby("deployment_id").size().to_dict()
    mismatches = sorted(
        (d, expected[d], int(actual.get(d, 0)))
        for d in expected
        if int(actual.get(d, 0)) != expected[d]
    )
    if mismatches:
        raise ValueError(
            "deployments.csv replica counts do not match pods.csv pod counts "
            "per deployment_id. Each Deployment.replicas value must equal the "
            "number of rows in pods.csv with that deployment_id, so the "
            "gang-placement IC `sum(Pod.placed).per(Deployment) == "
            "Deployment.replicas * Deployment.placed` is satisfiable. "
            f"Mismatches (deployment_id, expected_replicas, actual_pods): "
            f"{mismatches}"
        )


def _assert_one_zone_per_rack(nodes_df, source):
    rack_zones = nodes_df.groupby("rack")["zone"].nunique()
    bad_racks = sorted(rack_zones[rack_zones > 1].index.tolist())
    if bad_racks:
        raise ValueError(
            f"{source} has rack names appearing in multiple zones: "
            f"{bad_racks}. Each rack must belong to exactly one zone "
            f"(the Rack concept is identified by name); reusing a rack "
            f"name across zones would create an ambiguous Rack.zone "
            f"mapping."
        )


def _assert_no_self_pairs(df, col_a, col_b, source):
    self_rows = df[df[col_a] == df[col_b]]
    if not self_rows.empty:
        bad = sorted(self_rows[col_a].unique().tolist())
        raise ValueError(
            f"{source} has self-pair rows where {col_a} == {col_b} = {bad}. "
            f"Each pair must reference two distinct rows; a self-pair turns "
            f"the corresponding IC into a degenerate constraint (anti-"
            f"affinity → at most one pod of that tenant per node; affinity "
            f"→ all pods of that deployment collapsed onto one node)."
        )


_assert_unique_keys(nodes_csv, "node_id", "nodes.csv")
_assert_unique_keys(tenants_csv, "tenant_id", "tenants.csv")
_assert_unique_keys(deployments_csv, "deployment_id", "deployments.csv")
_assert_unique_keys(pods_csv, "pod_id", "pods.csv")
_assert_unique_keys(aa_csv, ["tenant_a", "tenant_b"], "tenant_anti_affinity.csv")
_assert_unique_keys(da_csv, ["deployment_a", "deployment_b"], "deployment_affinity.csv")
_assert_unique_keys(dt_csv, ["pod_a", "pod_b"], "distributed_training.csv")
_assert_no_self_pairs(aa_csv, "tenant_a", "tenant_b", "tenant_anti_affinity.csv")
_assert_no_self_pairs(da_csv, "deployment_a", "deployment_b", "deployment_affinity.csv")
_assert_no_self_pairs(dt_csv, "pod_a", "pod_b", "distributed_training.csv")

# Foreign-key edges in the schema. Update this table when you add a
# new event table or rewire a FK; the loop below validates every edge
# in one place.
_FK_EDGES = [
    (
        deployments_csv,
    ...
    (dt_csv, "pod_a", pods_csv, "pod_id", "distributed_training.csv", "pods.csv"),
    (dt_csv, "pod_b", pods_csv, "pod_id", "distributed_training.csv", "pods.csv"),
]
for _cdf, _ccol, _pdf, _pcol, _csrc, _psrc in _FK_EDGES:
    _assert_no_dangling_fks(_cdf, _ccol, _pdf, _pcol, _csrc, _psrc)

_assert_positive(nodes_csv, ["cpu_millicores", "memory_mib"], "nodes.csv")
_assert_non_negative(nodes_csv, "gpu_units", "nodes.csv")
_assert_positive(deployments_csv, "replicas", "deployments.csv")
_assert_non_negative(pods_csv, ["cpu_millicores", "memory_mib", "gpu_units"], "pods.csv")
_assert_replicas_match(deployments_csv, pods_csv)
_assert_one_zone_per_rack(nodes_csv, "nodes.csv")
