...
Four reasoner stages on a shared ontology:
  ...
  Stage 4 -- Prescriptive MIP: assigns workloads under power, GPU-count,
             ...
             across a scenario sweep (PowerEnvelopeLevel x MarginFloor x
             DiversityCap = 2 x 3 x 4 = 24 cells). Maximizes a four-factor strategic
...
Output:
    ...
    per-cell summary table (with INFEASIBLE cells as designed diagnostic signal), two Pareto frontiers
...
# tighter cells in the 24-cell sweep remain queryable via Assignment.x_assign
...
# Stage 4 solver. HiGHS is the open-source default that ships with the
# prescriptive engine; switch to "gurobi" if your engine has a Gurobi
# license (faster convergence, tighter MIP gaps on the 24-cell sweep).
SOLVER = "gurobi"
SOLVER_TIME_LIMIT_SEC = 240
...
# --------------------------------------------------
# Stage 0: Define semantic model & load data
# --------------------------------------------------
...
#   that the workload will be high-utilization next period. Stranded
#   capacity (depreciation accruing without offsetting revenue) is the
#   operator's biggest economic exposure, so this is the load-bearing
#   forward-looking signal.
#
#   Heterogeneous graph with three cross-concept edge types:
#     1. lab_workloads: LabMetric -> Workload owned by the same lab
#     2. WorkloadDependency.blocks DAG (shared with Stage 3)
#     3. co_dated: LabMetric -> LabMetric on the same date when their
#        labs share a workload_type (cross-lab industry co-movement)
#
#   Falls back to workload_utilization_fallback.csv when --no-gnn is set
#   or the predictive engine is unavailable. Downstream stages don't care
#   which path produced the probability.
# --------------------------------------------------
...
def _train_gnn_and_predict():
    ...
    lm_src = model.data(lab_metrics_df)
    model.define(LabMetric.new(
        metric_id=lm_src.metric_id, metric_date=lm_src.metric_date, lab=lm_src.lab,
        active_training_runs=lm_src.active_training_runs,
        gpu_hours_consumed=lm_src.gpu_hours_consumed,
        tokens_trained=lm_src.tokens_trained,
        eval_runs_launched=lm_src.eval_runs_launched,
        inference_qps=lm_src.inference_qps,
        model_release_events=lm_src.model_release_events,
        paper_submissions=lm_src.paper_submissions,
        funding_announced_usd=lm_src.funding_announced_usd,
        prev_day_growth=lm_src.prev_day_growth,
        prev_week_growth=lm_src.prev_week_growth,
        growth_7d_mean=lm_src.growth_7d_mean,
    ))

    # Heterogeneous graph (three cross-concept edge types). All edges cross
    # concept boundaries -- same-entity-only edges (e.g. pure temporal lag)
    # would not give the GNN message-passing lift over a tabular model.
    ...
    # share a workload_type. LOAD-BEARING -- carries cross-lab co-movement
    # signal a per-workload tabular baseline cannot replicate.
    _lab_name = ai_labs_df.set_index("id")["name"].to_dict()
    _types_by_lab = (
        workloads_df.assign(lab_name=workloads_df["lab_id"].map(_lab_name))
        .groupby("lab_name")["workload_type"].apply(set)
        .to_dict()
    )
    _labs = sorted(_types_by_lab.keys())
    _dates = sorted(lab_metrics_df["metric_date"].unique())
    co_pairs = pd.DataFrame([
        {"lab_a": a, "lab_b": b, "shared_date": d}
        for a in _labs for b in _labs
        if a != b and _types_by_lab[a] & _types_by_lab[b]
        for d in _dates
    ])
    ...
    # Train: 7 months x 110 = 770 obs; Val: 1 month x 110 = 110;
    # Test: current month x 110 (NO labels -- predict for all workloads).
    ...
    # datetime= per the rai-predictive-training triple-coupling rule.
...
if not args.no_gnn:
    if args.gnn_strict:
        util_df = _train_gnn_and_predict()
...
util_src = model.data(util_df)
WlRef = Workload.ref()
model.where(WlRef.id == util_src.workload_id).define(
    WlRef.utilization_probability(util_src.utilization_probability)
)
...
#   Hardware compatibility (memory + GPU-type allowlist) + priority tier
#   from contract_tier. Asymmetric failure-mode pricing: under-provisioning
#   an anchor (P0) carries an SLA / contract / reputational penalty that is
#   a multiple of the raw foregone revenue; Stage 4 multiplies the
#   assignment reward by (1 + under_provisioning_penalty).
# --------------------------------------------------
...
# Asymmetric failure mode: unfilled P0 seats hurt more than unfilled P2.
...
# A direct aggs.count(Workload).per(Workload.priority_tier) query trips
# the engine's functional-dependency check when Workload also carries
# the multi-valued compatible_with relationship -- selecting just two
...
#   Reverse-PageRank on the WorkloadDependency.blocks DAG: a high
#   `gating_score` means many downstream workloads are gated by this
#   one (frontier pretrain that gates 14 fine-tunes + evals lands at
#   the top). Workloads not in the DAG get baseline 1.0 via backstop.
# --------------------------------------------------
...
n_workloads_total = int(model.select(aggs.count(Workload).alias("n")).to_df()["n"].iloc[0])

if n_with_score < n_workloads_total:
...
#                      DiversityCap), binary. One per (Workload, GpuPool)
#                      in Compatibility, per scenario cell.
#
#   Constraints:
#     C1+C2 -- At most one assignment per workload per cell.
#              (Hard P0 commitment relaxed to <=1; priority_weight=100 in
#               the objective dominates so P0 is pushed to 1 wherever
#               feasible. Per-cell P0 drops surface as the diagnostic
#               signal the design specifies.)
...
#     C7    -- Workload-type floor (one per type; skipped when floor < 0).
#
#   Objective: maximize sum over assignments of
#       priority_weight * gating_score * utilization_probability *
#       strategic_value_usd * (1 + under_provisioning_penalty)
# --------------------------------------------------

print(f"\n{'=' * 60}\nSTAGE 4: PRESCRIPTIVE -- compute allocation MIP (24-cell sweep)\n{'=' * 60}")

# Decision variable: Assignment indexed by the three scenario axes
...
# an unfilled anchor seat (P0) costs more than the foregone revenue, while an
# unfilled research eval costs only the revenue. P0 = 1.0 (2x amp), P1 = 0.3, P2 = 0.0.
x_obj = Float.ref()
problem.maximize(
    sum(x_obj
...
# --------------------------------------------------
# Stage 4 (cont.): Per-cell report + headline-plan persistence
#   Per-cell aggregates queried directly from the ontology via
#   `sum(...).where(...).per(env, mar, div)` (mirrors energy_grid_planning's
#   `rev_per_level` pattern). Infeasibility is detected by the absence of
#   Assignment rows in a cell. The chosen baseline cell is persisted as an
#   AllocationPlan singleton + Assignment.is_chosen flag + DemandScenario
#   overlay so the plan is queryable as ontology after the script exits.
# --------------------------------------------------
...
if len(cells_df) == 0:
    ...
    # Per-(cell, workload_type) and per-(cell, priority_tier) counts.
    for wt in WORKLOAD_TYPES:
    ...
    # Per-cell status: cells with assignments are OPTIMAL; cells with none
    # are INFEASIBLE when the global solve proved optimality, or UNSOLVED when
    # the wall-clock ran out before the solver could explore that cell.
    _empty_label = "INFEASIBLE" if si.termination_status == "OPTIMAL" else "UNSOLVED"
    cells_df["status"] = cells_df["n_assigned"].apply(
        lambda n: "OPTIMAL" if n > 0 else _empty_label
    )
    cells_df = cells_df.sort_values(["envelope", "margin", "diversity"])

    n_cells_total = len(cells_df)
    n_feasible = (cells_df["status"] == "OPTIMAL").sum()
    n_empty = n_cells_total - n_feasible
    print(f"\n  Per-cell summary ({n_cells_total} cells: {n_feasible} optimal, {n_empty} {_empty_label.lower()}):")
    ...
    # (revenue, cost, anchor share, n_assigned, status, binding axis) for
    # the chosen cell; Assignment.is_chosen narrows the per-cell decision
    # rows to the chosen scenario. Mirrors the telco_network_recovery
    # RestorePlan / is_selected_upgrade pattern.
    chosen_row = cells_df[
    ...
    if len(chosen_row) == 0:
        ...
        # DemandScenario overlay: stranded-capacity exposure under risk.
        # The chosen plan is locked in, but lab-side demand for the assigned
        # workloads can soften from the GNN's point forecast. P0 revenue is
        # contractual (factor = 1.0); P1/P2 realize only `factor`. The delta
        # is stranded capacity: GPUs powered, depreciation accruing, no workload.
