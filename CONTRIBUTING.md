...
- The client-side depth cap is gone. A deeply nested expression no longer prints `<expression too deep>`; it renders on the engine instead. On the SQL backend, render time climbs steeply with nesting depth, so a template that builds very deep expressions will spend real time on the first query after an install.
